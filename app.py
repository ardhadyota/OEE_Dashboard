...
if uploaded_file is not None:
    try:
        ...
        if df_filtered.empty:
            st.warning("Tidak ada data produksi yang cocok dengan filter yang dipilih.")
            st.stop()

        # Menentukan Standar Aktif Sesuai Filter Line
        if selected_line == "Semua Line":
            active_std = DEFAULT_OVERALL_STD
        else:
            active_std = LINE_STANDARDS.get(selected_line, DEFAULT_OVERALL_STD)
