...
st.markdown(
    ...
    <div class="dashboard-header">
        <div><h1>OEE Executive Analytics</h1><p>Diagnosis Performa & Kondisi Operasional Line Produksi</p></div>
...
if uploaded_file is not None:
    try:
        ...
        with col_left:
            st.markdown(
                '<div class="section-title">B.1 Target vs Aktual OEE Semua Line</div>',
                unsafe_allow_html=True,
            )
...
# =========================================================
# VISUALISASI & ANALISIS KERUGIAN INTERAKTIF (PLOTLY)
# =========================================================
st.markdown("---")
st.header("📊 Visualisasi & Analisis Kerugian Interaktif")

# 1. WATERFALL CHART (OEE BREAKDOWN)
st.subheader("1. OEE Loss Breakdown (Waterfall)")

avg_avail = pd.to_numeric(df_filtered['% Availibility'], errors='coerce').mean() if '% Availibility' in df_filtered.columns else 1.0
avg_perf = pd.to_numeric(df_filtered['% Performance'], errors='coerce').mean() if '% Performance' in df_filtered.columns else 1.0
avg_qual = pd.to_numeric(df_filtered['Quality'], errors='coerce').mean() if 'Quality' in df_filtered.columns else 1.0

avail_pct = avg_avail * 100.0 if avg_avail <= 1.0 else avg_avail
perf_pct = avg_perf * 100.0 if avg_perf <= 1.0 else avg_perf
qual_pct = avg_qual * 100.0 if avg_qual <= 1.0 else avg_qual

avail_loss = 100.0 - avail_pct
perf_loss = avail_pct * (1.0 - (perf_pct / 100.0))
qual_loss = (avail_pct * (perf_pct / 100.0)) * (1.0 - (qual_pct / 100.0))
final_oee = (avail_pct / 100.0) * (perf_pct / 100.0) * (qual_pct / 100.0) * 100.0

fig_waterfall = go.Figure(go.Waterfall(
    name = "OEE", orientation = "v",
    measure = ["relative", "relative", "relative", "relative", "total"],
    x = ["Ideal OEE", "Availability Loss", "Performance Loss", "Quality Loss", "Actual OEE"],
    textposition = "outside",
    text = ["100%", f"-{avail_loss:.1f}%", f"-{perf_loss:.1f}%", f"-{qual_loss:.1f}%", f"{final_oee:.1f}%"],
    y = [100, -avail_loss, -perf_loss, -qual_loss, 0],
...
fig_waterfall.update_layout(
    title="Alur Degradasi OEE dari Standar Ideal (100%)",
    yaxis_title="Persentase (%)",
    showlegend = False
)
st.plotly_chart(fig_waterfall, use_container_width=True)

# 2. PARETO CHART (DOWNTIME LOSSES)
st.subheader("2. Pareto Chart Downtime (Prinsip 80/20)")

downtime_summary = {
    'Unplanned Downtime': df_filtered['Unplanned Downtime'].sum() if 'Unplanned Downtime' in df_filtered.columns else 0,
    'Setup & Adjustment': df_filtered['Setup & Adjustment'].sum() if 'Setup & Adjustment' in df_filtered.columns else 0,
    'Speed Losses': df_filtered['Speed Losses'].sum() if 'Speed Losses' in df_filtered.columns else 0
}

df_pareto = pd.DataFrame(list(downtime_summary.items()), columns=['Kategori', 'Menit'])
df_pareto = df_pareto.sort_values(by='Menit', ascending=False).reset_index(drop=True)
df_pareto['Kumulatif_Menit'] = df_pareto['Menit'].cumsum()
df_pareto['Kumulatif_Pct'] = (df_pareto['Kumulatif_Menit'] / df_pareto['Menit'].sum()) * 100 if df_pareto['Menit'].sum() > 0 else 0

fig_pareto = go.Figure()
fig_pareto.add_trace(go.Bar(
    x=df_pareto['Kategori'], y=df_pareto['Menit'],
    name='Durasi Loss (Menit)', marker_color='#636EFA'
))
fig_pareto.add_trace(go.Scatter(
    x=df_pareto['Kategori'], y=df_pareto['Kumulatif_Pct'],
    name='Kumulatif (%)', yaxis='y2', mode='lines+markers', line=dict(color='#FFA15A', width=3)
))

fig_pareto.update_layout(
    title="Kategori Downtime & Speed Loss Terbesar",
    yaxis=dict(title="Durasi (Menit)"),
    yaxis2=dict(title="Persentase Kumulatif (%)", overlaying='y', side='right', range=[0, 105]),
    legend=dict(x=0.6, y=1.1, orientation="h")
)
st.plotly_chart(fig_pareto, use_container_width=True)

# 3. HEATMAP DOWNTIME PER LINE & SHIFT
if 'Shift' in df_filtered.columns and 'LineID' in df_filtered.columns:
    st.subheader("3. Heatmap Kerugian Waktu (Line vs Shift)")
    
    df_filtered_copy = df_filtered.copy()
    df_filtered_copy['Total_Loss_Min'] = (
        df_filtered_copy.get('Unplanned Downtime', 0) + 
        df_filtered_copy.get('Setup & Adjustment', 0) + 
        df_filtered_copy.get('Speed Losses', 0)
    )
    
    heatmap_data = df_filtered_copy.pivot_table(
        index='LineID', 
        columns='Shift', 
    ...
    fig_heatmap = px.imshow(
        heatmap_data,
        labels=dict(x="Shift Kerja", y="Line Production", color="Total Loss (Menit)"),
        x=heatmap_data.columns,
        y=heatmap_data.index,
        color_continuous_scale="Reds",
        aspect="auto"
    )
    fig_heatmap.update_layout(title="Sebaran Kerugian Waktu per Line dan Shift")
...
# Ambil rata-rata performa agregat dari data yang difilter
avg_avail = pd.to_numeric(df_filtered['% Availibility'], errors='coerce').mean() if '% Availibility' in df_filtered.columns else 1.0
avg_perf = pd.to_numeric(df_filtered['% Performance'], errors='coerce').mean() if '% Performance' in df_filtered.columns else 1.0
avg_qual = pd.to_numeric(df_filtered['Quality'], errors='coerce').mean() if 'Quality' in df_filtered.columns else 1.0
...
# Menyiapkan data kerugian waktu
downtime_summary = {
    'Unplanned Downtime': df_filtered['Unplanned Downtime'].sum() if 'Unplanned Downtime' in df_filtered.columns else 0,
    'Setup & Adjustment': df_filtered['Setup & Adjustment'].sum() if 'Setup & Adjustment' in df_filtered.columns else 0,
    'Speed Losses': df_filtered['Speed Losses'].sum() if 'Speed Losses' in df_filtered.columns else 0
...
fig_pareto.add_trace(go.Scatter(
    x=df_pareto['Kategori'], y=df_pareto['Kumulatif_Pct'],
    name='Kumulatif (%)', yaxis='y2', mode='lines+markers', line=color='#FFA15A', line_width=3
))
...
if 'Shift' in df_filtered.columns and 'LineID' in df_filtered.columns:
    ...
    # Hitung total loss gabungan per Line dan Shift
    df_filtered['Total_Loss_Min'] = (
        df_filtered.get('Unplanned Downtime', 0) + 
        df_filtered.get('Setup & Adjustment', 0) + 
        df_filtered.get('Speed Losses', 0)
    )
